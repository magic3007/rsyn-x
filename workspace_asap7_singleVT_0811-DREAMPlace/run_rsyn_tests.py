...
def get_unique_output_dir(base_dir, git_hash):
    ...
    counter = 0
    while True:
        dir_name = f"rsyn_results/{git_hash}_{counter}"
...
def main():
    ...
    # --- Main Logic ---
    log(f"Searching for '*.rsyn' files in '{rsyn_scripts_dir}'...")
    rsyn_scripts = sorted(list(rsyn_scripts_dir.glob("*.rsyn")))

    if not rsyn_scripts:
        log("No '*.rsyn' scripts found.")
        sys.exit(0)
    ...
    for rsyn_script in rsyn_scripts:
        ...
        base_name = rsyn_script.name.replace('.rsyn', '')
