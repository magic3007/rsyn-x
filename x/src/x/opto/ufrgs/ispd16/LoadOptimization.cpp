...
bool LoadOptimization::run(const Rsyn::Json &params) {
	...
	const std::string optType = params.count("dontMoveRegistersAndLCBs")?
		params["type"] : "driver";
	const bool optionDontMoveRegistersAndLCBs = params.count("dontMoveRegistersAndLCBs")?
		params.count("dontMoveRegistersAndLCBs") : true;	
	
	if (optType== "steiner") {
		runMoveNonCriticalSinksOfCriticalNetsToSteinerPoint(optionDontMoveRegistersAndLCBs);
	} else if (optType == "driver") {
		runMoveNonCriticalSinksOfCriticalNetsToDriver(optionDontMoveRegistersAndLCBs);
	} else {
...
void LoadOptimization::runMoveNonCriticalSinksOfCriticalNetsToSteinerPoint(const bool dontMoveFlopsAndLCBs) {
	Stepwatch watch("Load optimization");

	Rsyn::Attribute<Rsyn::Instance, bool> visited = design.createAttribute();
	...
	for (Rsyn::Net net : module.allNets()) {
		orderedNets.push_back(
				std::make_tuple(
					timer->getNetCriticality(net, Rsyn::LATE)*routingEstimator->getRCTree(net).getTotalWireCap(), net));
	} // end for
	...
	for (int i = numElements - 1; i >= 0; i--) {
		...
		if (!driver || timer->getPinWorstSlack(driver, Rsyn::LATE) >= 0 ||
		...
		for (Rsyn::Pin sink : net.allPins(Rsyn::SINK)) {
			...
			if (cell.isFixed() || 
					(dontMoveFlopsAndLCBs && (cell.isLCB() || cell.isSequential())))
			...
			for (Rsyn::Pin pin : cell.allPins(Rsyn::OUT)) {
				if (timer->getPinWorstSlack(pin, Rsyn::LATE) < lateSlackThreshold || 
						timer->getPinWorstSlack(pin, Rsyn::EARLY) < earlySlackThreshold) {
					isCritical = true;
					break;
				} // end if
			} // end if
...
void LoadOptimization::runMoveNonCriticalSinksOfCriticalNetsToDriver(const bool dontMoveFlopsAndLCBs) {
	Stepwatch watch("Load optimization");
	...
	for (int i = numElements - 1; i >= 0; i--) {
		...
		for (Rsyn::Pin sink : net.allPins(Rsyn::SINK)) {
			...
			if (!isCritical) {
				...
				const double dy = lowerDriverPos[Y] - lowerCellPos[Y];
				double oldCost;
				double newCost;

				if (!infra->translateCellWithCostEvaluation(cell, (DBU) dx, (DBU) dy,
						LEG_NEAREST_WHITESPACE, COST_LOCAL_DELAY, Rsyn::LATE, oldCost, newCost)){
					counterLegalizationFail++;
				} // end if
